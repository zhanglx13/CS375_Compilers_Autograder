...
- On machines using a newer version of the linker (ld-2.34), ld complains about 
  multiple definition of `EOFFLG` and `CHARCLASS[MAXCHARCLASS]`.
  This is because they are defined in `lexan.h`, which is included in multiple
  source files (`scanner.c`, `lexanc.c`, `lexandr.c`). Therefore, the linker
  can see multiple definitions from these source files.
  To fix it, you can add `extern` to the definition of `EOFFLG` and `CHARCLASS[MAXCHARCLASS]`
  in `lexan.h` to make them declarations. Since the linker is OK with multiple
  declarations, it won't complain anymore.
