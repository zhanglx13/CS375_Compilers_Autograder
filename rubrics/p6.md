...
```
==> testNN (p) No Assembly Code Generated!!
```
without printing any further information.
And if there is a seg fault, the autograder will print
```
==> testNN (p) No Assembly Code Generated!! (Seg fault)
```
without printing any further information.

If your compiler generates at least one line of assembly code between
`begin Your code` and `begin Epilogue code`, the autograder will print the following
sections according to the mode.
